...
async def main():
    ...
    if _debate_on:
        handshake_instructions += (
            "\n\nThis server supports multi-model debate on ALL tools (debug, codereview, planner, "
            "chat, analyze, secaudit, etc.). When the user wants multiple AI perspectives, "
            "set debate_mode=true on the tool call. This sends the same prompt to 3 different LLMs "
            "in parallel, then has them critique each other, then synthesizes the result.\n\n"
            "WHEN TO USE debate_mode=true:\n"
            "- User says 'debate', 'multi-model', 'get multiple opinions', 'adversarial'\n"
            "- User says 'consensus' or 'get consensus' (use chat with debate_mode=true, debate_preset='debate' — "
            "this is BETTER than the old consensus tool because it includes adversarial critique)\n"
            "- User says 'review' or 'code review' (use codereview with debate_mode=true)\n"
            "- User says 'plan' or 'design' (use planner with debate_mode=true)\n"
            "- User says 'debug' or 'investigate' (use debug with debate_mode=true)\n"
            "- User says 'security audit' (use secaudit with debate_mode=true)\n"
            "- User says 'analyze' (use analyze with debate_mode=true)\n"
            "- ANY tool request where the user wants diverse perspectives\n\n"
            "DEFAULT BEHAVIOR: When the user just asks a question without specifying single vs multi-model, "
            "prefer debate_mode=true with debate_preset='debate' for substantive questions "
            "(architecture decisions, code reviews, debugging, planning). Use single-model (no debate_mode) "
            "for simple lookups, quick questions, and mechanical tasks.\n\n"
            "PRESETS (use debate_preset parameter):\n"
            "- 'ensemble'/'pick_best': 3 models answer, best one selected (Config B — fast)\n"
            "- 'debate'/'adversarial': 2-round adversarial debate (Config C — default for reviews)\n"
            "- 'full'/'research': debate + models request additional context between rounds (Config D)\n"
            "- 'quick'/'parallel': 3 models answer in parallel, synthesized, no critique (fastest multi-model)\n\n"
            "EXAMPLES:\n"
            "- 'review this code' → codereview with debate_mode=true, debate_preset='debate'\n"
            "- 'plan the implementation' → planner with debate_mode=true, debate_preset='debate'\n"
            "- 'debug this issue' → debug with debate_mode=true, debate_preset='debate'\n"
            "- 'consensus on X' → chat with debate_mode=true, debate_preset='debate'\n"
            "- 'get consensus' → chat with debate_mode=true, debate_preset='debate'\n"
            "- 'quick opinion on X' → chat with debate_mode=true, debate_preset='quick'\n"
            "- 'deep research on Y' → chat with debate_mode=true, debate_preset='full'\n"
            "- 'ensemble review' → codereview with debate_mode=true, debate_preset='ensemble'\n\n"
            "CRITICAL: When debate_mode=true, ALWAYS include it in the tool call parameters. "
