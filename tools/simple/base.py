...
class SimpleTool(BaseTool):
    ...
    async def _run_debate(self, request, prompt: str, system_prompt: str) -> Optional[str]:
        """
        Route a tool call through the shared debate routing module.
        Returns JSON string or None to fall through to single-model path.
        """
        from debate.routing import route_through_debate

        return await route_through_debate(
        ...
        try:
            ...
            if debate_mode and _cfg.DEBATE_FEATURE_ENABLED:
                ...
                if debate_result is not None:
                    # Return debate result — skip single-model path
                    # debate_result is a JSON string; wrap as TextContent
                    from mcp.types import TextContent
                    return [TextContent(type="text", text=debate_result)]
                # None means no models configured — fall through to single-model
